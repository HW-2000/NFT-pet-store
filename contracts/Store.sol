...
contract Store {
    ...
    function nftMintBuy(uint256 price, string memory tokenURI) external {
        // specify that only the contract owner can use this function
        require(
            msg.sender == NFT.owner(),
            "Only the NFT contract owner can call this function"
        );
        require(
        ...
        // mint NFT and transfer price-equivalent tokens
    ...
    function nftBuy(uint256 tokenId) public {
        ...
        require(
            token.allowance(msg.sender, NFT.ownerOf(tokenId)) >= nftPrice,
            "Insufficient allowance."
        );
        require(
        ...
        // transfer Paw token with nftPrice amount to NFT's owner
        ...
        // revert token state to not for sale
        tokenIdForSale[tokenId] = false;
